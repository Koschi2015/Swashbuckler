...
void CSpriteSheet::Load(std::string Pfad,int SpriteSize_X,int SpriteSize_Y)
{
    ...
    if(Managed)
    {
        ...
        Image_Y = SpriteSheetImg->GetHeight();
        Image_X = SpriteSheetImg->GetWidth();
        ...
        #endif
        SpriteSheet->SetImage(*SpriteSheetImg);
    }
    else
    {
        Path = Pfad;
...
sf::Sprite CAnimateSprite::operator() ()
{
    if(Animation_On)
    {
        if(CurrentFrame == -1)
        {
            ...
            TimeNextFrame = Timer.GetElapsedTime() + FrameTime;
            return Sheet->Get_Sprite(CurrentFrame);
        }
        else
        {
            if(Timer.GetElapsedTime() > TimeNextFrame)
            {
                CurrentFrame++;
                TimeNextFrame = Timer.GetElapsedTime() + FrameTime;
