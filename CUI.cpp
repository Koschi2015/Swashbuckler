...
const sf::Input* CUI_Surface::UI_InputHandle = 0;
...
{
    Scope.Bottom = BottomCoord;
    Scope.Left = LeftCoord;
    Scope.Right = RightCoord;
    Scope.Top = TopCoord;
}
...
{
    return X <= Scope.Right-Offset_X && X >= Scope.Left-Offset_X && Y <= Scope.Bottom-Offset_Y && Y >= Scope.Top-Offset_Y;
}
...
void CUI_Text::Load(std::string Pfad,float size,sf::String::Style Style)
{
    /*if(Font && FontPath == "")
    {
        delete Font;
...
{
    Text.SetText(text);
    Set_Scope(static_cast<int>(Text.GetRect().Left),static_cast<int>(Text.GetRect().Right),static_cast<int>(Text.GetRect().Top),static_cast<int>(Text.GetRect().Bottom));
}
void CUI_Text::Set_Style(sf::String::Style style)
{
    Text.SetStyle(style);
}
...
{
    if(DebounceTimer.GetElapsedTime() > 0.015f) // 15 ms Verzögerung
    {
        DebounceTimer.Reset();
        First_Element();
        do
        {
            if(Surface->UI_Element->MouseInside(UI_InputHandle->GetMouseX(),UI_InputHandle->GetMouseY()))
            {

                if(CUI_Surface::UI_InputHandle->IsMouseButtonDown(sf::Mouse::Left))
                {
                ...
                else if(CUI_Surface::UI_InputHandle->IsMouseButtonDown(sf::Mouse::Right))
                {
