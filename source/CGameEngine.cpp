...
CGameEngine::CGameEngine()
{
    ...
    InputHandleMouse = Window.Get_MouseInput_as_Pointer();
    // Übergeben des InputHandlers
    InputHandleKeyboard = Window.Get_KeyboardInput_as_Pointer();
    // Erstellen des EventHandlers
    EventHandler = new sf::Event;
    // Maus ausschalten
    Switch_Mouse_On(false);
    // InputHandle für UI
    CUI_Surface::Set_Input(InputHandleMouse);
    // MessageLoop für UI
    CUI_Surface::Set_MessageLoop(&Loop);
    //neue Mouse Icon
    Mouse = new CMouse(InputHandleMouse);
...
CState CGameEngine::GameLoop()
{
    ...
    while (Window.Get_Window_as_Pointer()->isOpen())
     {
         while (Window.Get_Window_as_Pointer()->pollEvent(*EventHandler))
         {
             // Close window : exit
         ...
         if(InputHandleKeyboard->isKeyPressed(sf::Keyboard::S))
         {
             Y++;
             std::cout << "S";
             SceneGraph->Set_Map_Center(X,Y);
         }
         if(InputHandleKeyboard->isKeyPressed(sf::Keyboard::W))
         {
             Y--;
             std::cout << "W";
             SceneGraph->Set_Map_Center(X,Y);
         }
         if(InputHandleKeyboard->isKeyPressed(sf::Keyboard::D))
         {
             X++;
             std::cout << "D";
             SceneGraph->Set_Map_Center(X,Y);
         }
         if(InputHandleKeyboard->isKeyPressed(sf::Keyboard::A))
         {
             X--;
             std::cout << "A";
             SceneGraph->Set_Map_Center(X,Y);
         }
         if(InputHandleKeyboard->isKeyPressed(sf::Keyboard::B))
         {
...
CState CGameEngine::MainMenuLoop()
{
    ...
    while (Window.Get_Window_as_Pointer()->isOpen())
    {
         ...
         if(InputHandleKeyboard->isKeyPressed(sf::Keyboard::Escape))
         {
             nState.Set_State(SM_REMOVELASTSTATE);
             nState.Set_Transparency(false);
             return nState;
...
void CGameEngine::Init_MainMenu()
{
    if(MainMenu_UI == 0)
    {
        ...
        Button.Set_Position(Window.Get_Window_as_Pointer()->getSize().x / 2 - 128 , (Window.Get_Window_as_Pointer()->getSize().y - 4*128) / 2);
        MainMenu_UI->Add_Element(Button);
        Button.Set_ArrayNummber(2);
        Button.Set_LMB_Msg("OptionKlick");
        Button.Set_Position(Window.Get_Window_as_Pointer()->getSize().x / 2 - 128 , (Window.Get_Window_as_Pointer()->getSize().y - 4*128) / 2 + 128);
        MainMenu_UI->Add_Element(Button);
        Button.Set_ArrayNummber(4);
        Button.Set_LMB_Msg("IntroKlick");
        Button.Set_Position(Window.Get_Window_as_Pointer()->getSize().x / 2 - 128 , (Window.Get_Window_as_Pointer()->getSize().y - 4*128) / 2 + 256);
        MainMenu_UI->Add_Element(Button);
        Button.Set_ArrayNummber(3);
        Button.Set_LMB_Msg("EndeKlick");
        Button.Set_Position(Window.Get_Window_as_Pointer()->getSize().x / 2 - 128 , (Window.Get_Window_as_Pointer()->getSize().y - 4*128) / 2 + 384);
        MainMenu_UI->Add_Element(Button);
...
void CGameEngine::Init_OptionMenu()
{
    if(Option_UI == 0)
    {
        ...
        Text.Load("Font/ModerneFraktur.ttf",29.0f,sf::Text::Regular);
...
void CGameEngine::Init_GameUI()
{
    if(Game_UI == 0)
    {
        ...
        Text.Set_Style(sf::Text::Underlined);
        Text.Set_Text("Test");
        Text.Set_Size(11.0);
        Game_UI->Add_Element(Text);
        GameStateStack.Get_State()->Set_Init(true);
