...
{
    Image->Set_Display_Coord(MouseInput->getPosition().x,MouseInput->getPosition().y);
    return MouseIcon->Get_CurrentSprite();
}
sf::Sprite CMouse::Mouse_Update(int X,int Y)
{
    Image->Set_Display_Coord(MouseInput->getPosition().x + X,MouseInput->getPosition().y + Y);
    return MouseIcon->Get_CurrentSprite();
}
...
CMouse::CMouse(const sf::Mouse* Input)
{
    #ifdef LOGFILE
    if(LogFile)
    {
